 ...
 * Drop-in replacement for RuntimeSession — same public interface, same events.
 *
 * Instead of spawning a CLI process and managing stdio pipes directly,
 * this delegates to the SDK which handles process lifecycle, message parsing,
 * and multi-turn conversations internally.
 *
 * Key differences from RuntimeSession:
 * - No Bun.spawn, no stdin/stdout, no NDJSON buffer parsing
 * - Uses query() with AsyncIterable<SDKUserMessage> for multi-turn
 * - Interrupt via query.interrupt() instead of control_request
 * - Thinking via query.setMaxThinkingTokens() instead of control_request
 * - Permission mode via query.setPermissionMode() instead of control_request
 * - Tool results pushed through message channel (not stdin)
 *
 * The SDK yields SDKPartialAssistantMessage (type: "stream_event") with the
 * same inner Anthropic SSE events (message_start, content_block_delta, etc.)
 * that RuntimeSession parses from stdout — so routeMessage() is nearly identical.
 */
...
import type { CreateSessionOptions, ResumeSessionOptions, StreamEvent } from "./session";
...
export class SDKSession extends EventEmitter {
   ...
   *
   * In SDK mode, "healthy" means the query generator is alive and the
   * message channel is open. "stale" uses the same 5-minute inactivity
   * threshold as RuntimeSession.
   */
  getInfo() {
    return {
      id: this.id,
   ...
   * Equivalent to RuntimeSession.ensureProcess().
   */
  ...
  private buildQueryOptions(): Record<string, unknown> {
    ...
    return {
      ...
      // Permissions — Phase 1: bypass everything (same as current)
      ...
      // Phase 1: auto-approve all tool calls (same as bypassPermissions)
   ...
   * Nearly identical to RuntimeSession.routeMessage(), matching on msg.type.
   *
   * The SDK yields the same types as the CLI stdout NDJSON:
   * - stream_event (SDKPartialAssistantMessage) — granular Anthropic SSE events
   * - assistant — complete/partial assistant message
   * - user — tool results from CLI execution
   * - result — query completion
   * - system — init, compaction, status
   *
   * The SDK does NOT yield control_response or keep_alive (handled internally).
   */
   ...
   * SDK yields: { type: "stream_event", event: { type: "content_block_delta", ... } }
   ...
   * Identical to RuntimeSession.trackEventState().
   */

      ...
      {messages.map((msg, msgIdx) => (
        ...
        >
          ...
          {msg.role === "user" ? (
            ...
            <>
              {(() => {
                // Group consecutive tool-like blocks (thinking + tool_use) together
                const groups: Array<{ type: 'single' | 'tool-group', blocks: any[], startIndex: number }> = [];
                ...
                for (let i = 0; i < msg.blocks.length; i++) {
                  ...
                  if (isToolLike) {
                    ...
                    currentGroup.push({ ...block, originalIndex: i });
                  } else {
                    // Non-tool block breaks the group
                    if (currentGroup.length > 0) {
                      groups.push({ type: 'tool-group', blocks: currentGroup, startIndex: groupStartIndex });
                      currentGroup = [];
                    }
                    groups.push({ type: 'single', blocks: [{ ...block, originalIndex: i }], startIndex: i });
                  }
                }
                ...
                return groups.map((group, groupIdx) => {
                  ...
                  // Tool group - render in flex container
                  return (
                    <div key={`group-${groupIdx}`} className="flex flex-wrap gap-2">
